 ...
 * люба firstName – любое имя, строка
 * lastName –я фамилия, строка
...
function defUpperStr(txt) {
  return (txt || 'Default text').toUpperCase();
}
...
function ageClassification(n) {
  var res = '';
  res = n < 0 ? null : n <= 24 ? 'детский возраст' : n <= 44 ? 'молодой возраст' : n <= 65 ? 'средний возраст' : n <= 75 ? 'пожилой возраст' : n <= 90 ? 'старческий возраст' : n <= 122 ? 'долгожители' : null;
  return res;
}
